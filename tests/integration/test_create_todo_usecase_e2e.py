...
Tests the complete flow from UseCase → Repository → Database
using real database connections and SQLAlchemy implementations.
"""
...
class TestCreateTodoUseCaseE2E:
    ...
    def test_create_todo_success_minimal_data(
        self,
        ...
        # Arrange
        usecase = CreateTodoUseCase(todo_repository, user_repository)
        title = "Complete project documentation"

        # Act
        created_todo = usecase.execute(
            title=title,
        ...
        assert created_todo.id is not None
        persisted_todo = todo_repository.find_by_id(created_todo.id)
        assert persisted_todo is not None
        assert persisted_todo.title == title
        assert persisted_todo.user_id == test_user_id

    def test_create_todo_success_complete_data(
        self,
        todo_repository: TodoRepository,
        user_repository: UserRepository,
        test_user_id: int,
    ...
    def test_create_multiple_todos_for_same_user(
        self,
        todo_repository: TodoRepository,
        user_repository: UserRepository,
        test_user_id: int,
        ...
        created_todos = []
        for title in titles:
            todo = usecase.execute(title=title, user_id=test_user_id)
    ...
    def test_create_todo_failure_user_not_found(
        ...
        all_todos = todo_repository.find_all_by_user_id(non_existent_user_id)
    ...
    def test_create_todo_priority_variations(
        self,
        todo_repository: TodoRepository,
        user_repository: UserRepository,
        test_user_id: int,
    ...
    def test_database_transaction_integrity(
        ...
        # Get initial count
        initial_todos = todo_repository.find_all_by_user_id(test_user_id)
        ...
        final_todos = todo_repository.find_all_by_user_id(test_user_id)
    ...
    def test_create_todo_with_multiple_users(
        self,
        todo_repository: TodoRepository,
        user_repository: UserRepository,
        test_user_id: int,
        ...
        saved_user2 = user_repository.save(user2)

        usecase = CreateTodoUseCase(todo_repository, user_repository)

        # Act - Create todos for different users
        todo1 = usecase.execute(title="User 1 todo", user_id=test_user_id)

        assert saved_user2.id is not None
        todo2 = usecase.execute(title="User 2 todo", user_id=saved_user2.id)

        # Assert - Verify todos were created with correct user_id
        assert todo1.user_id == test_user_id
        assert todo2.user_id == saved_user2.id

        # Verify todos exist in database
        assert todo1.id is not None
        assert todo2.id is not None
        persisted_todo1 = todo_repository.find_by_id(todo1.id)
        persisted_todo2 = todo_repository.find_by_id(todo2.id)

        assert persisted_todo1 is not None
        assert persisted_todo2 is not None
        assert persisted_todo1.user_id == test_user_id
